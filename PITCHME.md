...
A brief recap of his ICFP 2018 talk.
### Background
...
- Adds 'parallel composition' `<|>` as opposed to sequential and a product type (pairs), allowing to form 'closed cartesian category' abstraction and uses compiler plugin to compile (via categories) to efficient computation graphs (matrices?)
- Shows reverse mode as just left-association of composition (continuation passing style).
...
- Unifies reverse-mode AD as a special case, providing potential for parallelizable implementations
---
### Links
